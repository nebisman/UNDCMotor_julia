...
# ── 6. Función de transferencia nominal ──────────────────────────────
# Modelo predefinido del motor (no requiere conexión al hardware)
Gpos = transfer_function(sys; output=:position, min_order=true)
Gvel = transfer_function(sys; output=:speed, min_order=true)
println("\nFunción de transferencia (posición): $Gpos")
println("Función de transferencia (velocidad): $Gvel")

# ── 7. Diagrama de Bode del modelo identificado ─────────────────────
bodeplot(Gvel; title="Diagrama de Bode – Velocidad")
