...
#  Equivalente a motorsys.py.   LB 2026 – MIT License
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
#  Estructura principal:  MotorSystemIoT
# ═══════════════════════════════════════════════════════════════════════════════

"""
    MotorSystemIoT(; plant_number, port, bauds)

Interfaz serial con el sistema UNDCMotor (ESP32).
Equivalente a la clase MotorSystemIoT de Python.
"""
mutable struct MotorSystemIoT
    plant_number :: String
    port         :: String
    bauds        :: Int
    ...
    function MotorSystemIoT(;
            plant_number::String = "LEO",
        ...
        topics = Dict(
            "set_pid"       => "$pn/set_pid",
            "set_ref"       => "$pn/set_ref",
            "step_closed"   => "$pn/step_closed",
            "stairs_closed" => "$pn/stairs_closed",
            "prbs_open"     => "$pn/prbs_open",
            "step_open"     => "$pn/step_open",
            "set_gencon"    => "$pn/set_gencon",
            "prof_closed"   => "$pn/prof_closed",
        )
        new(plant_number, port, bauds, nothing, topics,
...
function connect!(sys::MotorSystemIoT)
    if sys.sp === nothing
...
function disconnect!(sys::MotorSystemIoT)
    _stop_reader!(sys)
    if sys.sp !== nothing
        close(sys.sp)
        sys.sp = nothing
...
function _start_reader!(sys::MotorSystemIoT)
...
function _stop_reader!(sys::MotorSystemIoT)
...
function _reader_loop(sys::MotorSystemIoT)
    buf = IOBuffer()
...
"""
function send_command!(sys::MotorSystemIoT, topic_key::String, payload::Dict)
...
"""
function receive_frames!(sys::MotorSystemIoT, total_frames::Int, on_frame::Function;
...
"""
function transfer_function(sys::MotorSystemIoT;
...
"""
function speed_from_volts(sys::MotorSystemIoT, volts::Real)
    u, y = read_csv_file()
...
"""
function volts_from_speed(sys::MotorSystemIoT, speed::Real)
    u, y = read_csv_file()
    speed == 0 && return 0.0
